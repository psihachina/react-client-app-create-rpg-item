...
export class ItemTypeDelete extends Component {
    constructor(props) {
        super(props);
    ...
    onSubmitForm = async (e) => {
        e.preventDefault();
        try {
            await fetch("http://localhost:8081/itemType/" + this.props.id, {
                method: "DELETE"
            });
    ...
    render() {
        return (
            <div>
                <Button color="ligth" onClick={this.toggle} className="p-0"><img width="65px" alt="Add" src="../trash-can.png" /></Button>
                <Modal isOpen={this.state.modal} toggle={this.toggle} className="btn" centered>
                    <ModalHeader toggle={this.toggle}>Delete item type</ModalHeader>
                    <ModalBody>
